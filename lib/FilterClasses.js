'use strict'
const crURL = require('./Utils').crURL

const Filter = {
  elemhideRegExp: /^([^\/\*\|\@"!]*?)##(.+)$/, // white elementHide filter is not supported
...
class RegExpFilter extends ActiveFilter {
  ...
  matches (location, contentType, docDomain) {
    ...
    return (this.regexp.test(location) &&
      ...
      (RegExpFilter.typeMap.get(contentType) & this.contentType) &&
      (typeof this.thirdParty === 'undefined' ||
      this.thirdParty === crURL.isThirdParty(crURL.extractDomain(location), docDomain))
    )
  }
...
module.exports = { Filter, ActiveFilter, RegExpFilter, BlockingFilter, WhitelistFilter, ElemHideFilter }
